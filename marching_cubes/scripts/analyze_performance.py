...
#fig_title_1 = "Relativní délky běhů konfigurací Loop algoritmu"
#fig_title_2 = "Relativní součty délek běhů konfigurací Loop algoritmu"
#directory = "../avs-proj02/loop_setup"

fig_title_1 = "Relativní délky běhů konfigurací Octree algoritmu"
fig_title_2 = "Relativní součty normalizovaných délek běhů konfigurací Octree algoritmu"
directory = "../avs-proj02/tree_setup"
...
for file in glob.glob(os.path.join(directory, "*.txt")):
    ...
    for row in df_mean.items():
        if row[0] not in mean_dict:
            mean_dict[row[0]] = []
        mean_dict[row[0]].append((file_name, row[1]))

        if "32." in file:
            if row[0] not in mean_32_dict:
                mean_32_dict[row[0]] = []
            mean_32_dict[row[0]].append((file_name, row[1]))
...
for key in mean_dict:
    mean_dict[key].sort(key=lambda x: x[1])
for key in mean_32_dict:
    mean_32_dict[key].sort(key=lambda x: x[1])
...
plt.legend()
plt.ylabel("Relativní délka běhu")
plt.xlabel("Vstupní soubor")
plt.tight_layout()
...
plt.ylabel("Relativní součet délek běhů")
plt.xlabel("Konfigurace")
plt.tight_layout()
plt.savefig(f"{directory}/relative_bar.png", dpi=500)
plt.show()

df_sorted = pd.DataFrame.from_dict(relative_dict, orient='index', columns=['Relativní součet délek běhů']).sort_values(by=['Relativní součet délek běhů'])
